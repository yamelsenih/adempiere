...
public class WPOSBPartner extends Window implements EventListener, ValueChangeListener
{
	...
	private WPOSTextField 		fTwitter = new WPOSTextField(null, null);
	private WPOSTextField 		fFacebook = new WPOSTextField(null, null);
	private WPOSTextField 		fInstagram = new WPOSTextField(null, null);
	...
	private void initBPartner()
	{
		...
		// PersonType
		fPersonType.getComponent().setWidth("97%");
		fPersonType.getComponent().setStyle(WPOS.FONTSIZESMALL);
	...
	public boolean loadBPartner (int C_BPartner_ID)
	{
		...
		fPersonType.setValue(partner.get_ValueAsString("PersonType"));
		...
		if (m_user != null)
		{
			...
			socialMediaList.forEach(item -> {
				...
				if(appSupport.equals(fTwitter.getId()))
					fTwitter.setText(item.getAccountName());
				else if(appSupport.equals(fFacebook.getId()))
					fFacebook.setText(item.getAccountName());
				else if(appSupport.equals(fInstagram.getId()))
	...
	private boolean actionSave()
	{
		...
		partner.set_CustomColumn("PersonType", itemPersonType);
