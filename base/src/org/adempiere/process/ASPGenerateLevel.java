...
public class ASPGenerateLevel extends ASPGenerateLevelAbstract {
	...
	private void addNodeToLevel(MTreeNode nn) {
		...
		if (menu.getAction().equals(MMenu.ACTION_Window)) {
			...
			if (asp_window_id < 1) {
				...
				aspWindow.setASP_Level_ID(levelId);
			...
			if (DB.getSQLValueEx(
					...
					levelId, form.getAD_Form_ID()) < 1) {
				X_ASP_Form aspForm = new X_ASP_Form(getCtx(), 0, get_TrxName());
				aspForm.setASP_Level_ID(levelId);
			...
			if (DB.getSQLValueEx(
				...
				aspBrowse.setASP_Level_ID(levelId);
			...
			if (DB.getSQLValueEx(
					...
					levelId, task.getAD_Task_ID()) < 1) {
				X_ASP_Task aspTask = new X_ASP_Task(getCtx(), 0, get_TrxName());
				aspTask.setASP_Level_ID(levelId);
	...
	private void generateProcess(int p_AD_Process_ID) {
		...
		if (asp_process_id < 1) {
			...
			aspProcess.setASP_Level_ID(levelId);
	...
	private void generateWorkflow(int p_AD_Workflow_ID) {
		...
		if (DB.getSQLValueEx(
				...
				levelId, workflow.getAD_Workflow_ID()) < 1) {
			X_ASP_Workflow aspWorkflow = new X_ASP_Workflow(getCtx(), 0, get_TrxName());
			aspWorkflow.setASP_Level_ID(levelId);
