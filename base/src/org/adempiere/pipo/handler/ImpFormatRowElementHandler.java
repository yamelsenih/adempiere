...
public class ImpFormatRowElementHandler extends AbstractElementHandler {
	...
	public void startElement(Properties ctx, Element element) throws SAXException {
		...
		if (m_ImpFormat_row.save(getTrxName(ctx)) == true){		    	
			recordLog (ctx, 1, m_ImpFormat_row.getName(),"ImpFormatRow", m_ImpFormat_row.get_ID(),AD_Backup_ID, Object_Status,"AD_ImpFormat",get_IDWithColumn(ctx, "AD_Table", "TableName", "m_ImpFormat_row"));           		        		
		}
		else{
			recordLog (ctx, 0, m_ImpFormat_row.getName(),"ImpFormatRow", m_ImpFormat_row.get_ID(),AD_Backup_ID, Object_Status,"AD_ImpFormat",get_IDWithColumn(ctx, "AD_Table", "TableName", "m_ImpFormat_row"));
